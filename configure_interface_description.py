...
f=open('configure_junos/interface_description.j2')
...
for item in devices_list:
    ...
    f=open('configure_junos/interface_description_' + item['device-id'] + '.conf','w')
    f.write(my_template.render(host_vars))
    f.close()
    junos_conf = 'configure_junos/interface_description_' + item['device-id'] + '.conf'
    device.open()
    cfg=Config(device)
